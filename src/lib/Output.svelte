...
<div class="">
  ...
  {/if}-->
  {#if data.length !== 0}
    <p class="text-lg font-semibold">{getFormatted(data[0].lon, data[0].lat)}</p>
  {:else}
    <p class="text-red-500">Could not find city</p>
  {/if}
  <a href={data.length !== 0?`https://www.google.com/maps/@${data[0].lat},${data[0].lon},12z`: ''} target="_blank" rel="noreferrer" class="relative bottom-0 right-0 m-6 group">
    ...
    <div class="absolute hidden group-hover:block w-32 h-6 bg-red-500 bottom-0 rounded p-2 text-sm text-white font-semibold">Google Maps Link</div>
  </a>
</div>